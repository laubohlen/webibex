...
def saved_animal_selection_view(request):
    # catching forms from selecting animal in chip compare views
...
def save_landmarks_view(request):
    if request.method == "POST":
        ...
        x_horn_scaled = round(float(request.POST.get("horn_x")))
        y_horn_scaled = round(float(request.POST.get("horn_y")))
        x_eye_scaled = round(float(request.POST.get("eye_x")))
        y_eye_scaled = round(float(request.POST.get("eye_y")))
...
def default_chip_compare_view(request, oid):
    ...
    if not id_and_vec:
        ...
        # query with pk__in=[...], SQL’s IN clause does not preserve the Python list order. 
        # By injecting a Case/When into order_by() enforces precise order
...
def project_chip_compare_view(request, oid):
    ...
    # build base queryset
    gallery_qs = IbexChip.objects.select_related(
        'ibex_image__location__region', 'embedding'
    ).annotate(
        image_year=ExtractYear('ibex_image__created_at')
    ).filter(
...
def geographic_chip_compare_view(request, oid):
    ...
    if not id_and_vec:
        ...
        # unzip into two parallel lists and compute distance
        ids, vecs = zip(*id_and_vec)
        gallery_vectors = np.stack(vecs) # (N, D)
        diffs = gallery_vectors - query_embedding # (N, D)
        dists = np.linalg.norm(diffs, axis=1) # (N,)
